...
const TotalParking = () => {
  ...
  useEffect(() => {
    fetch("https://parking-app-server.vercel.app/available")
      .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    if (checkOutDate) {
      fetch(`https://parking-app-server.vercel.app/parking/${id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <div>
      <h2 className="text-red-500 text-3xl text-center">
        {" "}
        Available In Parking Vehicles:{available.length - services.length}{" "}
      </h2>
